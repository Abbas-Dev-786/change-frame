...
type TestTool = {
  name: string
  execute: (input: unknown, options?: { signal?: AbortSignal }) => Promise<ToolResponse> | ToolResponse
}
...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    const previousLoads = Number(window.name.replace("changedecision-loads:", "")) || 0
...
test("completes the WebMCP hero journey without document reloads", async ({ page }) => {
  await page.goto("/")
  await expect(page.getByRole("heading", { name: "ChangeDecision OS" })).toBeVisible()
  ...
  expect(response.success).toBe(true)
  await expect(page.getByRole("cell", { name: "Avoids CONSTRAINT-12" })).toBeVisible()
...
test("returns explicit conflicts and unsupported geometry without hidden mutation", async ({ page }) => {
  await page.goto("/")
  await executeTool(page, "evaluate_resolution_options", { expectedStateVersion: 1 })
...
test("restores a valid session, resets monotonically, and rejects corrupted persistence", async ({ page }) => {
  await page.goto("/")
  await executeTool(page, "evaluate_resolution_options", { expectedStateVersion: 1 })
  await page.reload()
  ...
  await page.evaluate(() => {
    window.sessionStorage.setItem("changedecision-os:decision-room:v2", "{corrupted")
  })
  await page.reload()
...
async function executeTool(
  ...
  return page.evaluate(
    async ({ toolName, toolInput }) => window.webMcpTestHarness.execute(toolName, toolInput),
...
async function expectToolNames(page: Page, expectedNames: string[]): Promise<void> {
  await expect.poll(
    () => page.evaluate(() => window.webMcpTestHarness.names()),
