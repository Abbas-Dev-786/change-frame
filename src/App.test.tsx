...
it("loads the decision room in the canonical investigating state", () => {
  ...
  expect(
    screen.getByText("HVAC duct conflicts with structural beam B14"),
  ).toBeVisible();
  expect(screen.getByText("Investigating")).toBeVisible();
  expect(
    screen.getByRole("heading", { name: "Agent Flight Recorder" }),
  ).toBeVisible();
...
it("materializes options and creates a keyboard coordinate constraint", () => {
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Open Agent Flight Recorder" }),
  );
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Close flight recorder" }),
  );
  ...
  fireEvent.click(
    screen.getByRole("button", { name: /create field constraint/i }),
  );

  expect(screen.getByText("Options available")).toBeVisible();
  expect(
    screen.getByText("OPTION-A - Reroute through Corridor C"),
  ).toBeVisible();
  expect(screen.getByText("Option Comparison")).toBeVisible();
  expect(screen.getByText("Decision note")).toBeVisible();
  expect(
    screen.getByText("Rejection reason: Requires engineering review"),
  ).toBeVisible();
  expect(
    screen.getByText("Rejected by reviewer: requires engineering review."),
  ).toBeVisible();
...
it("keeps approval human-only and renders the final draft change order", () => {
  ...
  expect(
    screen.getByText(
      "Balances constructability, cost, and milestone protection after the field constraint.",
    ),
  ).toBeVisible();
  ...
  expect(
    screen.getByText(/After · OPTION-A revision 2 clears constraint/),
  ).toBeVisible();
  ...
  expect(
    screen.getByRole("heading", { name: "Change Ripple X-Ray" }),
  ).toBeVisible();
  ...
  expect(useDecisionRoomStore.getState().stateVersion).toBe(
    versionBeforeReplay,
  );
  ...
  expect(
    screen.getByRole("button", { name: /approve decision/i }),
  ).toBeEnabled();
  expect(
    screen.getByText("DEC-019 is ready for human approval."),
  ).toBeVisible();
  expect(
  ...
  expect(
    screen.getByText(/DEC-019 \/ OPTION-A\.r2 \/ CONSTRAINT-12 \/ v6/),
  ).toBeVisible();
  expect(
    screen.getByRole("link", { name: /Flight provenance/i }),
  ...
  expect(
    screen.getByText("CO-007 is a draft only; no contract was executed."),
  ).toBeVisible();
});
...
it("uses strategy-aware language in the change ripple", () => {
  ...
  expect(
    screen.getByText("Uses the evaluated resized section on M-204."),
  ).toBeVisible();
});
...
it("renders hostile constraint labels as text and keeps keyboard preview transient", () => {
  ...
  fireEvent.focus(
    screen.getByText("OPTION-B - Resize duct section").closest("[tabindex]")!,
  );
  ...
  expect(useDecisionRoomStore.getState().stateVersion).toBe(
    beforePreviewVersion,
  );

  const hostileLabel = '<img src=x onerror="window.compromised=true">';
  fireEvent.change(screen.getByLabelText("Label"), {
    target: { value: hostileLabel },
  });
  fireEvent.click(
    screen.getByRole("button", { name: /create field constraint/i }),
  );

  expect(useDecisionRoomStore.getState().constraints[0]?.label).toBe(
    hostileLabel,
  );
